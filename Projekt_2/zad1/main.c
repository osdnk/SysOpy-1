...
void print_times(struct timeval *real_start, struct rusage *time_start,
                 struct timeval *real_end, struct rusage *time_end,
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    counter =
        getopt_long(argc, argv, ":F:n:s:C:o:t:", long_options, &option_index);
  ...
  for (int i = 0; i < ops; i++) messages[i] = calloc(ops * 2 + 1, sizeof(char));
  ...
  if (strcmp(op, "generate") == 0 && file_path && records_number &&
      block_size) {
  ...
  if (strcmp(op, "copy") == 0 && file_path && cp_to_path && records_number &&
      block_size) {
  ...
  if (strcmp(op, "sort") == 0 && file_path && cp_to_path && records_number &&
      block_size) {
  ...
  if (!generate__) {
    ...
    fprintf(fp, "\tBLOCK_SIZE: %d RECORDS_NUMBER: %d\n", block_size,
            records_number);
    print_times(&real_time[0], &usage_time[0], &real_time[1], &usage_time[1],
                messages[0], fp);
    fclose(fp);
  }

  for (int i = 0; i < ops; i++) free(messages[i]);

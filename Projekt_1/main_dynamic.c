...
void add_remove_blocks(int count, Table *tab, void *handle);
void add_remove_blocks_cross(int count, Table *tab, void *handle);
...
int main(int argc, char **argv)
{
    ...
    {
        Table *(*create_table)(unsigned int height, unsigned int width, bool is_static) = dlsym(handle, "create_table");
    ...
    for (int i = 3; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "ar") == 0)
        {
            ...
            times(ts);
            add_remove_blocks(count, table, handle);
            times(te);
            void print_times(struct tms * ts, struct tms * te);
        }
        else if (strcmp(argv[i], "arc") == 0)
        {
            ...
            times(ts);
            add_remove_blocks_cross(count, table, handle);
            times(te);
            void print_times(struct tms * ts, struct tms * te);
        }
    }
...
void add_remove_blocks(int quantity, Table *tab, void *handle)
{
    void (*delete_block)(int, Table *) = dlsym(handle, "delete_block");
...
void add_remove_blocks_cross(int quantity, Table *tab, void *handle)
{
    void (*delete_block)(int, Table *) = dlsym(handle, "delete_block");
    void (*create_block)(Table *, char *) = dlsym(handle, "create_block");
